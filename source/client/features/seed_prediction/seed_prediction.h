...
public:
	...
	bool is_perfect_shot(c_base_combat_weapon* weapon, float provided_time);
	bool dispatch_user_message_handler(bf_read* buf, int type);
	void apply_spread_correction(vector& angles, int seed, float spread);
	bool send_net_message_handler(i_net_message* data);
	void send_net_message_post_handler();
	void cl_sendmove_handler( );
	void cl_sendmove_handler_post();
	void reset_variables();
	void create_move_handler(c_user_cmd* cmd);
};

inline c_seed_prediction* seed_prediction = new c_seed_prediction;